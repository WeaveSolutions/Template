# PHP Laravel API Service Infrastructure
# This module manages the PHP Laravel API service deployment and infrastructure
  ...
  container_definitions = jsonencode([
    {
      ...
      logConfiguration = {
        logDriver = "awslogs"
        options = {
          awslogs-group         = aws_cloudwatch_log_group.php_api_logs[0].name
...
resource "aws_ecs_service" "php_api" {
  ...
  task_definition = aws_ecs_task_definition.php_api[0].arn
  ...
  network_configuration {
    ...
    security_groups  = [aws_security_group.php_api[0].id]
    assign_public_ip = false
  }
  
  load_balancer {
    target_group_arn = aws_lb_target_group.php_api[0].arn
    container_name   = local.service_name
    container_port   = local.container_port
  }
  
  service_registries {
    registry_arn = aws_service_discovery_service.php_api[0].arn
  }
...
resource "aws_appautoscaling_target" "php_api" {
  count = var.enable_php_api && var.enable_autoscaling ? 1 : 0
  
  max_capacity       = var.max_capacity
  min_capacity       = var.min_capacity
  resource_id        = "service/${var.ecs_cluster_name}/${aws_ecs_service.php_api[0].name}"
...
resource "aws_appautoscaling_policy" "php_api_cpu" {
  ...
  resource_id        = aws_appautoscaling_target.php_api[0].resource_id
  scalable_dimension = aws_appautoscaling_target.php_api[0].scalable_dimension
  service_namespace  = aws_appautoscaling_target.php_api[0].service_namespace
...
# Security Group
resource "aws_security_group" "php_api" {
  count = var.enable_php_api ? 1 : 0
...
resource "aws_cloudwatch_log_group" "php_api_logs" {
  count = var.enable_php_api ? 1 : 0
...
resource "aws_service_discovery_service" "php_api" {
  count = var.enable_php_api ? 1 : 0
...
resource "aws_ssm_parameter" "kong_service_config" {
  count = var.enable_php_api && var.enable_kong_integration ? 1 : 0
...
module "posthog_integration" {
  ...
  count  = var.enable_php_api && var.enable_posthog ? 1 : 0
