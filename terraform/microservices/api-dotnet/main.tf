# .NET ASP.NET Core API Service Infrastructure
# This module manages the .NET ASP.NET Core API service deployment and infrastructure
...
resource "aws_ecs_service" "dotnet_api" {
  ...
  task_definition = aws_ecs_task_definition.dotnet_api[0].arn
  ...
  network_configuration {
    ...
    security_groups  = [aws_security_group.dotnet_api[0].id]
    assign_public_ip = false
  }

  load_balancer {
    target_group_arn = aws_lb_target_group.dotnet_api[0].arn
    container_name   = local.service_name
    container_port   = local.container_port
  }

  service_registries {
    registry_arn = aws_service_discovery_service.dotnet_api[0].arn
  }

  depends_on = [
    aws_lb_listener_rule.dotnet_api,
...
# Security Group
resource "aws_security_group" "dotnet_api" {
  count = var.enable_dotnet_api ? 1 : 0
...
resource "aws_lb_target_group" "dotnet_api" {
  count = var.enable_dotnet_api ? 1 : 0
...
resource "aws_lb_listener_rule" "dotnet_api" {
  count = var.enable_dotnet_api ? 1 : 0
...
resource "aws_service_discovery_service" "dotnet_api" {
  count = var.enable_dotnet_api ? 1 : 0
...
resource "aws_cloudwatch_log_group" "dotnet_api" {
  count = var.enable_dotnet_api ? 1 : 0
