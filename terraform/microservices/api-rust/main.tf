# Rust Actix API Service Infrastructure
# This module manages the Rust Actix API service deployment and infrastructure
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        {
          name  = "DATABASE_URL"
          value = "postgresql://${var.database_username}@${var.database_host}:5432/${var.database_name}"
        },
        {
          name  = "REDIS_URL"
      ...
      logConfiguration = {
        logDriver = "awslogs"
        options = {
          awslogs-group         = aws_cloudwatch_log_group.rust_api_logs[0].name
...
resource "aws_ecs_service" "rust_api" {
  ...
  task_definition = aws_ecs_task_definition.rust_api[0].arn
  ...
  network_configuration {
    ...
    security_groups  = [aws_security_group.rust_api[0].id]
    assign_public_ip = false
  }
  
  load_balancer {
    target_group_arn = aws_lb_target_group.rust_api[0].arn
    container_name   = local.service_name
    container_port   = local.container_port
  }
  
  service_registries {
    registry_arn = aws_service_discovery_service.rust_api[0].arn
  }
...
# Auto Scaling Target
resource "aws_appautoscaling_target" "rust_api" {
  count = var.enable_rust_api && var.enable_autoscaling ? 1 : 0
  
  max_capacity       = var.max_capacity
  min_capacity       = var.min_capacity
  resource_id        = "service/${var.ecs_cluster_name}/${aws_ecs_service.rust_api[0].name}"
...
resource "aws_appautoscaling_policy" "rust_api_cpu" {
  count = var.enable_rust_api && var.enable_autoscaling ? 1 : 0
  
  name               = "${var.project_name}-${local.service_name}-cpu"
  policy_type        = "TargetTrackingScaling"
  resource_id        = aws_appautoscaling_target.rust_api[0].resource_id
  scalable_dimension = aws_appautoscaling_target.rust_api[0].scalable_dimension
  service_namespace  = aws_appautoscaling_target.rust_api[0].service_namespace
...
resource "aws_appautoscaling_policy" "rust_api_memory" {
  count = var.enable_rust_api && var.enable_autoscaling ? 1 : 0
...
# Security Group
resource "aws_security_group" "rust_api" {
  count = var.enable_rust_api ? 1 : 0
...
resource "aws_lb_target_group" "rust_api" {
  count = var.enable_rust_api ? 1 : 0
...
resource "aws_lb_listener_rule" "rust_api" {
  count = var.enable_rust_api ? 1 : 0
...
resource "aws_cloudwatch_log_group" "rust_api_logs" {
  count = var.enable_rust_api ? 1 : 0
...
resource "aws_service_discovery_service" "rust_api" {
  count = var.enable_rust_api ? 1 : 0
...
resource "aws_ssm_parameter" "kong_service_config" {
  count = var.enable_rust_api && var.enable_kong_integration ? 1 : 0
...
module "posthog_integration" {
  ...
  count  = var.enable_rust_api && var.enable_posthog ? 1 : 0
