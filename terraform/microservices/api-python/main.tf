# Python FastAPI Service Infrastructure
# This module manages the Python FastAPI service deployment and infrastructure
  ...
  container_definitions = jsonencode([
    {
      ...
      logConfiguration = {
        logDriver = "awslogs"
        options = {
          "awslogs-group"         = aws_cloudwatch_log_group.python_api[0].name
...
resource "aws_ecs_service" "python_api" {
  ...
  task_definition = aws_ecs_task_definition.python_api[0].arn
  desired_count   = var.desired_count
  launch_type     = "FARGATE"

  network_configuration {
    subnets          = var.private_subnet_ids
    security_groups  = [aws_security_group.python_api[0].id]
    assign_public_ip = false
  }

  load_balancer {
    target_group_arn = aws_lb_target_group.python_api[0].arn
...
# Security Group
resource "aws_security_group" "python_api" {
  count = var.enable_python_api ? 1 : 0
...
resource "aws_appautoscaling_target" "python_api" {
  count = var.enable_python_api && var.enable_auto_scaling ? 1 : 0
  
  max_capacity       = var.max_capacity
  min_capacity       = var.min_capacity
  resource_id        = "service/${var.ecs_cluster_name}/${aws_ecs_service.python_api[0].name}"
...
resource "aws_appautoscaling_policy" "python_api_cpu" {
  ...
  resource_id        = aws_appautoscaling_target.python_api[0].resource_id
  scalable_dimension = aws_appautoscaling_target.python_api[0].scalable_dimension
  service_namespace  = aws_appautoscaling_target.python_api[0].service_namespace
...
resource "aws_appautoscaling_policy" "python_api_memory" {
  count = var.enable_python_api && var.enable_auto_scaling ? 1 : 0
