# Tauri App Deployment Guide

## Overview

The Nexpo Tauri app is configured for automated cross-platform deployment using GitHub Actions and Tauri CLI. This guide covers local development, building, and deployment processes.
...
1. **Using Development Scripts**:
   ```bash
   # Windows PowerShell
   .\apps\tauri\scripts\dev.ps1
   
   # macOS/Linux
   ./apps/tauri/scripts/dev.sh
   ```
...
```env
# Enable Taurte platform for desktop development
ENABLE_TAURTE=true
ENABLE_TAURI_DESKTOP=true

# Optional: Target only Taurte platform for focused development
TARGET_PLATFORM=taurte
```
...
- `apps/tauri/out/make/` - Platform-specific installers
- `apps/tauri/out/` - Unpacked application

## Automated Deployment

### GitHub Actions Workflow

The deployment workflow (`.github/workflows/deploy-tauri.yml`) automatically builds and publishes releases when:
...
2. **Manual Trigger**: Via GitHub Actions UI
   - Navigate to Actions → Deploy Tauri App
   - Click "Run workflow"
...
3. **Enable in Workflow**:
   Uncomment the code signing sections in `deploy-tauri.yml`
...
The Tauri app includes auto-update functionality:
...
- [ ] Update version in `apps/tauri/package.json`
...
1. **Build Failures**:
   - Ensure all native dependencies are rebuilt: `pnpm tauri-rebuild`
...
Run Tauri with debug output:
```bash
# Windows
set DEBUG=tauri:* && pnpm start

# macOS/Linux
DEBUG=tauri:* pnpm start
```
...
{
  name: '@tauri-apps/maker-squirrel',
  config: {
    name: 'NexpoDesktop',
    authors: 'Nexpo Team',
    exe: 'nexpo.exe',
...
{
  "make:dev": "tauri build --config tauri.dev.config.ts",
  "make:prod": "tauri build --config tauri.prod.config.ts"
}
```
