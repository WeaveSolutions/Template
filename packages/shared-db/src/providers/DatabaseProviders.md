...
npm run db:generate -- --provider=postgres
...
npm run db:migrate -- --provider=postgres --name=init

# Reset the database (DANGER: This will drop all data)
npm run db:reset -- --provider=postgres
```

### Open Prisma Studio

```bash
npm run db:studio -- --provider=postgres
```
