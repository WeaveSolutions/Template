...
Examples:
  ...
  pnpm run seed:clear:provider postgres

  # Force clear without prompts
  pnpm run seed:clear:provider mongodb -- --force --yes

  # Dry run to see what would be cleared
  pnpm run seed:clear:provider supabase -- --dry-run

  # Production clear (requires FORCE_PRODUCTION_CLEAR=true)
  FORCE_PRODUCTION_CLEAR=true pnpm run seed:clear:provider sqlserver
`);
}
