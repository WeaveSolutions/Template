...
Examples:
  ...
  pnpm run seed:provider postgres

  # Dry run for MongoDB
  pnpm run seed:provider mongodb -- --dry-run

  # Force seed with custom counts
  SEED_USER_COUNT=100 pnpm run seed:provider supabase -- --force
`);
}
    ...
    if (!options.force) {
      try {
        ...
        if (stats.userCount > 0 || stats.accountCount > 0) {
          ...
          console.log(`   pnpm run seed:clear:provider ${provider}`);
          process.exit(1);
        }
      } catch (error) {
