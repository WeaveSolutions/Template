 ...
 */
export class SqlServerSyncAdapter {
    ditto = null;
    provider = null;
    config;
    isInitialized = false;
        ...
        try {
            ...
            if (syncDirection === SyncDirection.TO_CLOUD || syncDirection === SyncDirection.BIDIRECTIONAL) {
                ...
                for (const doc of dittoDocuments) {
                    ...
                    const existingResult = await this.provider.executeRawQuery(`SELECT Id FROM ${tableName} WHERE Id = @p1`, [doc.id]);
                    if (existingResult.length > 0) {
                        // Update existing document
                        await this.provider.executeRawQuery(`UPDATE ${tableName} SET Data = @p1 WHERE Id = @p2`, [JSON.stringify(doc.value), doc.id]);
                    }
                    else {
                        // Insert new document
                        await this.provider.executeRawQuery(`INSERT INTO ${tableName} (Id, Data) VALUES (@p1, @p2)`, [doc.id, JSON.stringify(doc.value)]);
                    }
                }
            }
...
//# sourceMappingURL=ditto-sqlserver-sync.js.map
