...
export class Auth0WebAuth implements AuthContextType {
  ...
  getEnabledProviders(): string[] {
    ...
    if (process.env.NEXT_PUBLIC_ENABLE_SNAPCHAT_LOGIN === 'true') {
      providers.push('snapchat');
    }
    if (process.env.NEXT_PUBLIC_ENABLE_INSTAGRAM_LOGIN === 'true') {
      providers.push('instagram');
    }
    if (process.env.NEXT_PUBLIC_ENABLE_TIKTOK_LOGIN === 'true') {
