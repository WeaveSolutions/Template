...
import { PostgresSyncAdapter } from './ditto-postgres-sync';
import { MongodbSyncAdapter } from './ditto-mongodb-sync';
import { CosmosdbSyncAdapter } from './ditto-cosmosdb-sync';
import { SqlServerSyncAdapter } from './ditto-sqlserver-sync';
import { IbmCloudSyncAdapter } from './ditto-ibmcloud-sync';
import { SupabaseSyncAdapter } from './ditto-supabase-sync';
import { FirebaseSyncAdapter } from './ditto-firebase-sync';
import { AwsRdsSyncAdapter } from './ditto-aws-rds-sync';
import { AwsS3SyncAdapter } from './ditto-aws-s3-sync';
/**
...
function isFeatureFlagEnabled(flagName) {
    ...
    if (typeof process !== 'undefined' && process.env) {
        if (process.env[flagName] === 'true')
            return true;
        if (process.env[`ENABLE_${flagName}`] === 'true')
            return true;
        if (process.env[`FEATURE_${flagName}`] === 'true')
            return true;
    }
    // Expo environment variables
    if (typeof process !== 'undefined' && process.env) {
        if (process.env[`EXPO_PUBLIC_${flagName}`] === 'true')
            return true;
        if (process.env[`EXPO_PUBLIC_ENABLE_${flagName}`] === 'true')
            return true;
        if (process.env[`EXPO_PUBLIC_FEATURE_${flagName}`] === 'true')
            return true;
    }
    return false;
}
...
//# sourceMappingURL=ditto-sync-factory.js.map
