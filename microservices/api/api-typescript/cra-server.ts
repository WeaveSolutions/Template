...
// CRA Configuration
import { getCRAConfig, validateCRAConfig, getCorsOrigins, type CRAConfig } from './config/cra-config';

// Initialize Express app
const app = express();
const PORT = parseInt(process.env.PORT || '7000', 10);

// Initialize CRA configuration
let craConfig: CRAConfig;
try {
  craConfig = getCRAConfig();
  validateCRAConfig(craConfig);
  console.log(`🚀 CRA initialized for platform(s): ${craConfig.platform}`);
  if (craConfig.nexpo?.enabled) {
    console.log('📱 Nexpo platform enabled (Next.js + Expo)');
  }
  if (craConfig.taurte?.enabled) {
    console.log('🖥️ Taurte platform enabled (Svelte + Tauri)');
    console.log(`🔧 Tauri desktop mode: ${craConfig.taurte.tauriDesktopMode}`);
  }
} catch (error) {
  console.error('❌ Failed to initialize CRA configuration:', error);
  process.exit(1);
}
...
function configureMiddleware(): void {
  ...
  // CORS configuration based on enabled platforms
  const corsOptions = {
    origin: function (origin: string | undefined, callback: (err: Error | null, allow?: boolean) => void) {
      // Allow requests with no origin (like mobile apps or curl requests)
      if (!origin) return callback(null, true);
      
      const allowedOrigins = getCorsOrigins(craConfig);
      
      if (allowedOrigins.includes(origin)) {
        callback(null, true);
      } else {
        console.warn(`🚫 CORS: Blocked origin: ${origin} (Platform: ${craConfig.platform})`);
  ...
  const limiter: RateLimitRequestHandler = rateLimit({
    windowMs: craConfig.shared.rateLimiting.windowMs,
    max: craConfig.shared.rateLimiting.max,
    message: {
      error: 'Too many requests from this IP, please try again later.',
      retryAfter: craConfig.shared.rateLimiting.windowMs / 1000
    },
...
function configureRoutes(): void {
   ...
   *     description: Returns version and feature information about the API
   *     responses:
   *       200:
